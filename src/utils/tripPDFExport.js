...
(Rider: ${riderName}) Tj
0 -20 Td
(Date: ${date.toLocaleDateString()}) Tj
0 -20 Td
(Time: ${date.toLocaleTimeString()}) Tj
0 -40 Td
/F2 16 Tf
(Trip Details) Tj
0 -30 Td
...
0 -20 Td
(Duration: ${Math.floor(duration / 60)}m ${duration % 60}s) Tj
0 -20 Td
...
0 -40 Td
/F2 16 Tf
(Eco-Score: ${ecoScore}/100) Tj
0 -30 Td
/F1 12 Tf
(Rating: ${ecoRating}) Tj
0 -20 Td
(Energy Efficiency: ${(100 - ecoScore)}% room for improvement) Tj
0 -40 Td
/F2 14 Tf
(Analysis) Tj
0 -25 Td
/F1 11 Tf
${ecoScore >= 80 
  ? '(Excellent! You rode efficiently. Keep smooth acceleration.)' 
  : ecoScore >= 60 
  ? '(Good ride. Cruise at 40 km/h for +20% range.)' 
  : '(Aggressive riding detected. Smooth acceleration saves ₹500/year.)'}
Tj
0 -20 Td
(Projected Range: ${(100 / batteryUsed * duration / 60).toFixed(0)} hours at full battery) Tj
ET
endstream
endobj
5 0 obj
...
export const downloadTripPDF = (tripData) => {
  ...
  link.href = url;
  link.download = `trip-${new Date().toISOString().split('T')[0]}.pdf`;
