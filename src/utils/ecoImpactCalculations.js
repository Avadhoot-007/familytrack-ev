...
const CO2_PER_KM_PETROL = 0.192;
const TREE_ABSORBS_PER_YEAR = 21;
const EV_EMISSIONS_FACTOR = 0.05;
...
export const getNextBadgeTarget = (totalCO2Saved) => {
  const thresholds = Object.values(BADGE_THRESHOLDS).sort((a, b) => a.co2 - b.co2);
...
export const getCoachingTips = (ecoScore = 0, worstAxis = 'speed', avgSpeed = 0, throttleData = [], distance = 0) => {
  ...
  if (worstAxis === 'throttle') {
    tips.push({
      priority: 'high', icon: '🎛️', title: 'Throttle Control',
    ...
    tips.push({
      priority: 'high', icon: '⚡', title: 'Speed Optimization',
    ...
    tips.push({
      priority: 'high', icon: '🚀', title: 'Acceleration Smoothness',
  ...
  if (ecoScore >= 90) {
    tips.push({ priority: 'info', icon: '🏆', title: 'Eco Champion Status!', tip: 'Excellent riding. Maintain this pattern for consistent range.', metric: `Score: ${ecoScore}/100` });
    tips.push({ priority: 'info', icon: '💚', title: 'Share Your Success', tip: 'Help others learn by sharing your eco-riding techniques.', metric: 'Community impact growing' });
  } else if (ecoScore >= 75) {
    tips.push({ priority: 'low', icon: '👍', title: 'Good Form', tip: 'Minor tweaks could unlock +10-15 points. Focus on steady speed.', metric: `Score: ${ecoScore}/100` });
  } else if (ecoScore < 50) {
    tips.push({ priority: 'critical', icon: '⚠️', title: 'Aggressive Riding', tip: 'Current pattern wastes 30%+ range. Ease off for 40% more miles.', metric: `Score: ${ecoScore}/100` });
  }

  if (distance > 10) {
    tips.push({ priority: 'info', icon: '📏', title: 'Long Trip Energy', tip: 'Cruise at steady 30 km/h for long rides. Variance drains battery faster.', metric: `Distance: ${distance.toFixed(1)} km` });
  }

  return tips.length > 0 ? tips : [{ priority: 'info', icon: '📊', title: 'Start Tracking', tip: 'Complete a trip to receive personalized coaching tips.', metric: 'No trips recorded yet' }];
};
...
export const generateImpactReport = (tripData) => {
  ...
  return {
    distance, duration, ecoScore, avgSpeed,
...
// ---------------------------------------------------------------------------
// PDF Export
// FIX 1: replaced non-ASCII chars (= U+2550, - U+2500, bullet U+2022)
//         with plain ASCII. Courier Type1 has no Unicode -> garbled output.
// FIX 2: dynamic /Length per stream (was hardcoded 2200 -> truncation).
// FIX 3: T* operator for line spacing (cleaner than repeated 0 -12 Td).
// ---------------------------------------------------------------------------
...
export const generateTripPDF = (tripData) => {
  ...
  for (let i = 0; i < pageCount; i++) {
    // T* moves down one leading (12pt) — cleaner than Td each line
    let stream = 'BT\n/F1 10 Tf\n50 750 Td\n12 TL\n';
    ...
    stream += 'ET\n';
    // FIX: exact byte length, not hardcoded
    objects.push(
