...
export default function RiderTipsInbox({ riderId }) {
  ...
  const markRead = async (tipKey) => {
    try {
      await update(ref(db, `riders/${riderId}/coachingTips/${tipKey}`), {
        read: true,
      });
    } catch (e) {
  ...
  const markAllRead = async () => {
    const unreadTips = tips.filter((t) => !t.read);
    await Promise.all(unreadTips.map((t) => markRead(t.key)));
  };

  // ── Clear all tips from Firebase ───────────────────────────────────────────
  const clearAllTips = async () => {
    try {
      await set(ref(db, `riders/${riderId}/coachingTips`), null);
    } catch (e) {
      console.error("clearAllTips error:", e);
    }
  };

  // ── Delete single tip ──────────────────────────────────────────────────────
  const deleteTip = async (tipKey) => {
    try {
      await set(ref(db, `riders/${riderId}/coachingTips/${tipKey}`), null);
    } catch (e) {
      console.error("deleteTip error:", e);
    }
  };
    ...
    <>
      ...
      <button
        ...
        style={{
          ...
          outline: "none",
          border:
            unread > 0 ? "2px solid rgba(255,152,0,0.6)" : "2px solid #333",
        }}
        title={
          unread > 0
            ? `${unread} unread tip${unread > 1 ? "s" : ""}`
            : "Coaching tips"
        }
      >
        💡
        ...
        >
          ...
          <div>
            ...
            <p style={{ margin: "2px 0 0", fontSize: "12px", color: "#666" }}>
              {tips.length === 0
                ? "No tips yet"
                : `${tips.length} tip${tips.length > 1 ? "s" : ""} from your watcher`}
            </p>
          </div>
